...
describe("Test Mediator", () => {
  ...
  it("should have an inistance of Mediator", () => {
    expect(mediator instanceof components.Mediator).to.be.true;
  });
  it("should trigger notification of trackClick single pos 89 = pos - 15 & setStep = 71.25", () => {
    mediator.notifyTrackClick(89);
    expect(mock.thumbModel.getPosition()).to.be.equal(71.25);
  });
  it("should setFill according to set position = 71.25", () => {
    expect(mock.fillModel.getFillPosition()).to.be.equal(71.25);
  });
  it("should trigger notification of trackClick array pos 89 => 55,150 the closest pos to thumb = [ 71.25, 150 ]", () => {
  ...
  it("should setFill according to set array position = [71.25, 150]", () => {
    const newPos = [71.25, 150];
